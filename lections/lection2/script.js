...
// если мы объявляем функцию вне объекта, в её this лежит глобальный объект Window
const checkThis = function () {
    console.log(this);
}
checkThis(); // Window {0: global, window: Window, self: Window,document: document, name: "", location: Location, …}

// если мы объявляем функцию внутри пользовательского объекта - в качестве this в функции мы получили наш объект
...
// Одалживание метода
// Объект Roomba - робот-пылесос. Код самого объекта смотри выше
...
// Объект Roomba - робот-пылесос. Код самого объекта смотри выше
// Обращение к свойствам и методом объекта Roomba. Код обращения выше
// Объект Tango - робот-пылесос. Код самого объекта смотри выше
...
// Давайте создадим третего робота, опять же отличающегося только свойствами, а функциональность будет та же
...
// Объект Samba - робот-пылесос. Код самого объекта смотри выше.
...
// I am cleaning... I have started: NaN times. ОШИБКА Количество запусков пылесоса стало NaN
// UV lamp is working. ОШИБКА ультрафиолетовая лампа не выключилась
// I am going to charge...

// Один из вариантов исправления обернуть метод в анонимную функцию, и вызвать в ней, тогда эта анонимная функция в своем лексическом окружении сохранит ссылку на объект, из которого наш метод будет вызываться:
// Объект Roomba - робот-пылесос. Код самого объекта смотри выше
// Обращение к свойствам объекта.
console.log(Roomba.model); // "Romba-1"
console.log(Roomba.isFull); // false
...
// Метод метод для привязки контекста call
// позволяет вызвать функцию и явно указать с каким объектом контекста её выполнить (передать в качестве первого аргумента объект, который будет доступен в функции через ключевое слово this).
// Объект Roomba - робот-пылесос. Код самого объекта смотри выше
// Обращение к свойствам объекта.
console.log(Roomba.model); // "Romba-1"
console.log(Roomba.isFull); // false
...
Roomba.startCleaning.call(notARobot1); // I am cleaning... I have been started: 11 times.

// метод метод для привязки контекста apply
// позволяет вызвать функцию и передать необходимый контекст, единственным отличием от call, метод apply принимает аргументы, которые необходимо передать в вызываемую функцию не списком через запятую, а в виде массива, что порой удобнее
// Объект Roomba - робот-пылесос. Код самого объекта смотри выше
...
// метод для привязки контекста bind
// позволяет привязать контекст к функции раз и навсегда, и в дальнейшем мы можем просто вызывать функции и быть уверены, что она будет вызвана в контексте нужного нам объекта
// Объект Roomba - робот-пылесос. Код самого объекта смотри выше
// Обращение к свойствам объекта.
console.log(Roomba.model); // "Romba-1"
console.log(Roomba.isFull); // false
